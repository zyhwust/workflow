    ...
    <div class="header">
    	<div class="header-input">
    		...
    		<div class="header-keywords">
    			<span>关键字：</span>
    			<input type="text" class="search-input" v-model="search_input" placeholder="请输入关键字搜索">
    		</div>
    		<!-- 输入时间范围 -->
    		<div class="header-timepick">
    			<span>时间范围：</span>
	    		<div class="time-wrap">
	    			<input v-model="dateStart" class="time-start" type="date"><span>-</span><input v-model="dateEnd" class="time-end" type="date">
	    		</div>
    		</div>    		
    		<button class="search-btn" @click="showDepResult(1)">搜索</button>
    		<div class="clear-float"></div>
    	</div>
    	<!-- 条件选择 -->
    	<div class="header-condition">
    		<div class="condition-check">
    			<span class="condition-title">部门科室：</span>
    			<span class="no-condition" @click="unlimited">不限</span>
    			<select v-model="depId">
	    			<option selected value="">请选择</option>
	    			<option v-for="dep in deps" v-bind:value="dep.deptId">{{dep.deptName}}</option>
	    		</select>
    		</div>
    		<div class="condition-check">
    			<span class="condition-title priority-title">优先级：</span>
    			<span class="no-condition" @click="unlimitedOne">不限</span>
    			<span v-for="item in priority" class="priority-radio" >
    				<input type="radio"  :value="item" v-model="pick1">
					<label>{{item.name}}</label>
    			</span>
    		</div>
    		<div class="condition-check">
    			<span class="condition-title">任务状态：</span>
    			<span class="no-condition" @click="unlimitedTwo">不限</span>
    			<span v-for="item in items" class="priority-radio">
    				<input type="radio"  :value="item" v-model="pick2">
					<label>{{item.name}}</label>
    			</span>
    		</div>
    		<div class="condition-check">
    			<span class="condition-title">任务类型：</span>
    			<span class="no-condition" @click="unlimitedThree">不限</span>
    			<span v-for="item in types" class="priority-radio">
    				<input type="radio"  :value="item" v-model="pick3">
					<label>{{item.name}}</label>
    			</span>	    		
    		</div>
    	</div>
    	<!-- 已经选择的关键字 -->
    	<div class="header-showkeyword">
    		<span>已选：</span>
			<span class="clear-either" v-if="pick1!=''" @click="clearEither(1)">{{pick1.name}}<span>×</span></span>
			<span class="clear-either" v-if="pick2!=''" @click="clearEither(2)">{{pick2.name}}<span>×</span></span>
			<span class="clear-either" v-if="pick3!=''" @click="clearEither(3)">{{pick3.name}}<span>×</span></span>
    		<!-- </span> -->
    		<span class="clear-all" @click="clearAll">全部清空</span>
    		<div class="clear-float"></div>
    	</div>
    </div>	   
    <div class="clear-float"></div> 
    ...
    <div class="task-body">
    	...
    	<div class="task-list">   	
		    <li v-for="item in tasks">
				<tasklist v-bind:model="item"></tasklist>
			</li>
    	</div>

    	<!-- 分页 -->		
		<div class="task-paging" v-if='hasDate'>
...
<script>
import mo_taskList from 'components/mo_taskList.vue';
	//接口返回数据
	var children = [];
	export default { 
		components:{
	        tasklist : mo_taskList,
	    },
	    replace:true, 		
    	data(){
    		return{
    			items:[{id:'0',name:'未完成',},{id:'1',name:'完成',},],//任务状态
    			types:'',//任务类型
    			deps:[],//公司架构
    			depId:'',//部门ID
    			priority:'',//优先级
    	...
    	events:{
    		'changeisopen':function(arg){
	            var vm = this;
	            vm.isOpen = arg[0];  //isOpen置为true
	            console.log(arg[1].model.name);
	            vm.taskId = arg[1].model.id;	            
	            vm.getTaskDetail(vm.taskId);
	        },
    	},
    	methods:{
    		unlimited:function(){
    		...
    		unlimitedOne:function(){
    			var vm = this;
    			console.log(vm.pick1.id);
    			vm.pick1 = '';
    		},
    		unlimitedTwo:function(){
    			var vm = this;
    			vm.pick2 = '';
    		},
    		unlimitedThree:function(){
    			var vm = this;
    			vm.pick3 = '';
    		},
    		// selectEither:function(){
    		// 	var vm = this;    			
    		// 	console.log(vm.pick1);
    		// },
    		clearEither:function(index){
    			var vm = this;
    			if(index == 1){
    				vm.unlimitedOne();
    			}else if(index == 2){
    		...
    		//综合搜索
    		showDepResult:function(curPage){
    			// alert(vm.depId);
    			var vm = this;	    			
    			var argObj = {
    				page:curPage,
    			...
    			if(vm.depId!=''){
    				argObj.createUserDeptId = vm.depId;
    			}
    			if(vm.dateStart!=''){
    				argObj.startTime = vm.dateStart;
    			}
    			if(vm.dateEnd!=''){   				
	    			argObj.endTime = vm.dateEnd;
    			}
    			if(vm.pick1!=''){
    				argObj.priorityId = vm.pick1.id;
    			}
    			if(vm.pick2!=''){
    				argObj.status = vm.pick2.id;
    			}
    			if(vm.pick3!=''){
    				argObj.taskTypeId = vm.pick3.id;
    			}
    			if(vm.search_input!=''){
    				argObj.searchString = vm.search_input;
    			}   
    			if(parseInt(vm.changeDate(vm.dateStart))>parseInt(vm.changeDate(vm.dateEnd))){
    				tools.toast("结束日期不能小于起始日期","default");
    				return;
    			} 
    			tools.busy();
				var obj = {
					url:'/home/task/1812.spring',
					args:argObj,
					success:function(rst){
						tools.busy(false);
						vm.tasks = rst.data;
						if(vm.tasks.length == 0){
							...
							//防XSS
							for (var i=0,len=vm.tasks.length; i < len; i++) {
								for(var key in vm.tasks[i]){
									vm.tasks[i][key] = encodeHtml(vm.tasks[i][key]);
								}
							}
							vm.hasDate = true;
						}
			...
			showSubtaskDetail:function(index){
				var vm = this;				
				// alert(vm.subtasks[index].id)
				$(".slideright").slideLeftHide();
				vm.taskId = vm.subtasks[index].id;
				vm.getTaskDetail();
			},
			// 获取任务详情
			getTaskDetail:function(arg){
				var vm = this;
			...
			subTask:function(){
				...
				var obj = {
					...
					success:function(rst){
						...
						vm.isPublish = false;
						//刷新子任务列表
		    			// vm.id = vm.subtasks[vm.operateIndex].id;
						var obj = {
		    				url:'/home/task/1803.spring',
		    				args:{
								id:vm.taskId
			...
			getDep:function(){
				...
				var obj = {
					...
					success:function(rst){
						vm.deps = rst.data;
						//防XSS
						for (var i=0,len=vm.deps.length; i < len; i++) {
							for(var key in vm.deps[i]){
								vm.deps[i][key] = encodeHtml(vm.deps[i][key]);
							}
						}
					},				
					asy:true
